...
install_path=$(dirname $(python -c "import os,sys; print(os.path.realpath(os.path.expanduser(sys.argv[1])))" "$0"))
...
case "$1" in
    ...
    "local-install-auto" )
        install_from download https://smartpy.io/dev-20200830-61244befe2c4c321c0ae6f807873e3a77811f20a/cli "$2" ;;
    "local-install-from" )
        shift
        install_from cp "$1" "$2" ;;
